...
# Dernière modification le : 22/07/2025                                                                #
# ---------------------------------------------------------------------------------------------------- #
...
def build_u_coeffs(u, v, p):
    ...
    Qp = -dy * (p[1:-1, 2:] - p[1:-1, :-2]) / 2

    b = Qt*u[1:-1, 1:-1] + Qp

    return aP, aE, aW, aN, aS, b

def build_v_coeffs(u, v, p):
    """
    Assemble the coefficients for the y-momentum (v-velocity) equation.
